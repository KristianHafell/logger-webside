...
describe('logTable-test', () => {
    ...
    const connection = { endpoint: 'https://jyb0pgmaq3.execute-api.eu-north-1.amazonaws.com/dev', ConnectionId: 'W1JeIfxkAi0CF7Q='};
    ...
    it('should set items in table', async () => {
        const items = await setItemsInTable(tableName, {id:"test", message: "test", metric: "test"});
        expect(items.statusCode).toEqual(200);
        console.log("items: ", items.body);
    });
    
    it('should return 404 if item not table', async () => {
        const items = await itemIsInTable(tableName, 'gsfhjjkadlwukbefsdcv');
        expect(items.statusCode).toEqual(404);
        console.log("items: ", items.body);
    });
    ...
    it('should return 500 if conecting to stream', async () => {
        const items = await streamConnectionOnline({ endpoint: 'https://wrong/dev', ConnectionId: 'wrong'});
        expect(items.statusCode).toEqual(500);
    });
    it('should return 200 if conecting to stream', async () => {
        const items = await streamConnectionOnline(connection);
        console.log("items: ", items.body);
        expect(items.statusCode).toEqual(200);
    });
